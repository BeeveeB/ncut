...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        @if(Request::get('id'))
            query_url ="{{route('practice_data_green' ,Request::get('id'))}}";
        @else
            query_url="{{route('practice_data_green' ,1)}}";
        @endif
        ...
        var interval = setInterval(function() {
            ...
            if (counter <= 0) {
                ...
                alert('Times Up');
                if (next_id <= 9) {
                    $( "#redirect_btn" ).html("<a href='{{route('menu_green')}}' class='btn btn-info mr-2 mt-2'>回關卡頁</a><a href='{{route('practice_green')}}?id=" + next_id + "' class='btn btn-info mr-2 mt-2'>下一關</a>");
                }
                else {
                    $( "#redirect_btn" ).html("<a href='{{route('menu_green')}}' class='btn btn-info mr-2 mt-2'>回關卡頁</a>");
                }
                return;
            }
            else if (counter <= 10){
                ...
                if (next_id <= 9) {
                    $( "#redirect_btn" ).html("<a href='{{route('menu_green')}}' class='btn btn-info mr-2 mt-2'>提早結束，回關卡頁</a><a href='{{route('practice_green')}}?id=" + next_id + "' class='btn btn-info mr-2 mt-2'>提早結束，跳下一關</a>");
                }
                else {
                    $( "#redirect_btn" ).html("<a href='{{route('menu_green')}}' class='btn btn-info mr-2 mt-2'>提早結束，回關卡頁</a>");
                }
            }
            else{
                $('#clock').text(counter+"秒");
